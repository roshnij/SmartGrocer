...
if __name__ == "__main__":
	...
	groceryIngredients = []
	grocIngrIDS = []
	for eachIngr in response_body:
		for prod,val in eachIngr.iteritems():
			if(prod =="product"):
				val = val.encode('utf-8')
				val = re.sub(r'\'','',val)
				groceryIngredients.append(val)
	
			if(prod =="id"):
				grocIngrIDS.append(val)
	...
	if(similarIngredient != ""):
		index = groceryIngredients.index(similarIngredient)
		print grocIngrIDS[index]
